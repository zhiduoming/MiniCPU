...
# Portable Vivado import

The repository is self-contained. Game renderer dependencies and image ROM
initialization files are under `vendor/game/sources_1`; no external
`digital_game_project` path is required.

On another computer, create an empty Vivado project for
`xc7a100tfgg484-1`, then run this in the Tcl Console:

```tcl
source {<path-to-cpu>/scripts/vivado_import.tcl}
```

The script selects `RV32I46F5SPMMIOSoCTOP` and sets `ROM_INIT_FILE` to the
absolute path of this copy's `mem/program.hex`. Regenerate synthesis,
implementation and the bitstream after importing.
