...
module HazardUnit (
    ...
    output reg [2:0] hazard_mem,
    output reg [2:0] hazard_wb,
    ...
    wire is_mac = (EX_opcode == `OPCODE_FENCE) && (EX_funct7 == 7'b0000001);
    wire mem_hazard_rs3 = is_mac && MEM_register_write_enable && (MEM_rd != 5'd0) && (MEM_rd == EX_rs3);
    wire wb_hazard_rs3 = is_mac && WB_register_write_enable && (WB_rd != 5'd0) && (WB_rd == EX_rs3);
    ...
    // Only CSRRS/CSRRC forms with a zero source are pure CSR reads. Treating
    // CSRRW and read-modify-write forms as reads deadlocks back-to-back setup
    // writes to mie, mstatus and mtvec.
    wire csr_read_in_id   = (ID_opcode == `OPCODE_ENVIRONMENT) &&
                            ((ID_funct3 == `CSR_CSRRS  || ID_funct3 == `CSR_CSRRC  ||
                              ID_funct3 == `CSR_CSRRSI || ID_funct3 == `CSR_CSRRCI) &&
                             (ID_rs1 == 5'b0));
    ...
    always @(*) begin
        ...
        hazard_mem[2] = mem_hazard_rs3;
        hazard_wb[2] = wb_hazard_rs3 && !mem_hazard_rs3;
